...
import static com.jacob.mringrtcdemo.CameraState.Direction.BACK;
import static com.jacob.mringrtcdemo.CameraState.Direction.FRONT;
import static com.jacob.mringrtcdemo.CameraState.Direction.NONE;
import static com.jacob.mringrtcdemo.CameraState.Direction.PENDING;
...
public class Camera implements CameraControl, CameraVideoCapturer.CameraSwitchHandler {
  ...
  private static class FilteredCamera2Enumerator extends Camera2Enumerator {
    ...
    private static final String TAG = Camera2Enumerator.class.getSimpleName();
    ...
    public @NonNull String[] getDeviceNames() {
      ...
      try {
        ...
        if (cameraManager != null) {
          ...
          for (String device: cameraManager.getCameraIdList()) {
            if (!isMonochrome(device, cameraManager)) {
              devices.add(device);
            }
          }
          ...
          for (String device: devices) {
            if (isLensFacing(device, cameraManager, CameraMetadata.LENS_FACING_FRONT)) {
              frontCamera = device;
              break;
            }
          }
          ...
          for (String device: devices) {
            if (isLensFacing(device, cameraManager, CameraMetadata.LENS_FACING_BACK)) {
              backCamera = device;
              break;
            }
          }
